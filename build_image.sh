...
if [ "$1" = "" ] || [ "$1" = "image" ]; then
	...
	$JAVA_HOME/bin/jlink --module-path "module_injected_jars/:target/mods:$JAVAFX_LOCATION/javafx-graphics/$JAVAFX_VER/:$JAVAFX_LOCATION/javafx-base/$JAVAFX_VER/:$JAVAFX_LOCATION/javafx-media/12.0.2/:$JAVAFX_LOCATION/javafx-swing/$JAVAFX_VER/:$JAVAFX_LOCATION/javafx-controls/$JAVAFX_VER/:$JAVA_HOME/jmods" --add-modules "org.darisadesigns.polyglotlina.polyglot","jdk.crypto.ec" --output "build/image/" --compress=2 --launcher PolyGlot=org.darisadesigns.polyglotlina.polyglot
fi

if [ "$1" = "" ] || [ "$1" = "pack" ]; then
  rm -rf appimage
  echo "packing mac app..."
  $JAVA_PACKAGER_LOCATION/jpackage --runtime-image build/image --input target --output appimage --name PolyGlot --main-jar $JAR_W_DEP --copyright "2014-2019 Draque Thompson" --description "PolyGlot is a spoken language construction toolkit." --mac-bundle-identifier "PolyGlot" --mac-bundle-name "PolyGlot" --file-associations packaging_files/mac/file_types_mac.prop --icon packaging_files/mac/PolyGlot.icns
  ...
  cp packaging_files/mac/PolyGlot.icns appimage/PolyGlot.app/Contents/Resources/PolyGlot.icns
  cp packaging_files/mac/PolyGlotFile.icns appimage/PolyGlot.app/Contents/Resources/PolyGlotFile.icns
fi
